...
When you update the source code for a given module or modules that have already been loaded and you want GHCi to recompile the program, you can use the `:reload` command. See https://downloads.haskell.org/~ghc/9.0.1/docs/html/users_guide/ghci.html#ghci-cmd-:reload[GHCi commands :reload] in the GHC documentation for details.
...
Note that the above effects also occur for the `:add` and `:reload` commands. See https://downloads.haskell.org/~ghc/9.0.1/docs/html/users_guide/ghci.html#the-effect-of-load-on-what-is-in-scope[The effect of :load on what is in scope] in the GHC documentation for more details.
