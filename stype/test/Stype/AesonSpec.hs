...
spec :: Spec
spec = do
  describe "encoding Continuous" $ do
    it "Inf encodes" $ toJSON (ContInf :: Continuous Double) `shouldBe` "Inf"
    it "Negative Inf encodes"
      $          toJSON (NegContInf :: Continuous Double)
      `shouldBe` "-Inf"
    it "Continuous encodes"
      $          toJSON (Cont 7.5 :: Continuous Double)
      `shouldBe` Number 7.5

  describe "encoding Nonnegative Continuous" $ do
    it "NonNegative Inf encodes"
