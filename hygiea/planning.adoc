...
My solution in `hygiea` marshals dhall into `Map Text TestAtomic` (and soon `TestVal` for the value type) where `TestAtomic` is an enum with variants for the particular types `v` I wanted to support. It is easy enough then to write a `FromDhall` instance for `TestAtomic`. That approach takes from dhall's `Expr` enum representing all possible dhall expressions.
