...
TODO: (I think) A sequence of *distinct* (non-overlapping) intervals and an addition-like operator can be defined as a monoid, a set closed under associative binary operator that has an identity element. NOTE: I think this is *not* a commutative monoid. 
...
[] + [\Xi] &= [\Xi] \\
[\Xi] + [\Xi'] &=
[head([\Xi])](last([\Xi]) + first([\Xi'])[tail([\Xi'])])
\end{align*}
$$
...
\end{cases}
$$
Note that $(\Xi, +)$ is *not* a monoid, but $([\Xi], +)$ is.


## Context

Define a context $\square$ as an [associative array](https://en.wikipedia.org/wiki/Associative_array).
...
U = \{ \square, W\}
$$
The context could contain information such as a patient ID and information that is constant over the course of the study as birth date and race. Note that this information could also be stored in event context(s) -- the decision is up to the study planners.
