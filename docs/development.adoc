...
Gaining familiarity with the Haskell module system is helpful in becoming proficient with the asclepias Haskell libraries. Furthermore,  To fully understand how the provided mechanisms work, we will first need a basic understanding of Haskell modules and how they are used to manage namespacing.
The following subsections provide a basic description of the Haskell module system. Please see the following documentation for more details.
...
2. The `makeBaselineFromIndex` entity is exported. Many types of entities are just a singular thing and thus don't need (and aren't allowed to) include the `(..)` syntax.
...
TODO: https://www.haskell.org/onlinereport/haskell2010/haskellch5.html#x11-990005.1
...
When you update the source code for a given module or modules that has already been loaded and you want GHCi to recompile the program, you can use the `:reload` command.
...
==== How `:load` affects scope in GHCi
...
https://downloads.haskell.org/~ghc/9.0.1/docs/html/users_guide/ghci.html#the-effect-of-load-on-what-is-in-scope
