...
A cohort builder is a function mapping a population and a specification to a cohort:

$$
\Upsilon: (P, S) \mapsto C 
$$
An analysis plan is a function $\Lambda$ that maps a specification $S$ to a cohort builder.


$$
\Lambda: S \mapsto ( C: P  \mapsto C )
$$
...
* $C'$ and the function types it is composed of
* $C$ and the data types it is composed of 

Before demostrating a complete working example starting from $S$ and $\Lambda$, we first explain how $C'$ works with toy example building a cohort "by hand" using the types 
