...
Therefore, the fundamental object in `hygiea` could be a typeclass
[source,haskell]
----
...
=== Test outputs are an internal, concrete `OutcomeData` type
Though test inputs need not be concrete types, there are two main reasons why the test output type, call it `OutcomeData`, probably should be. Doing so

1. provides a mechanism by which writers of test config files can specify tests without needing to know implementation details of project-specific types
2. gives a common structure for golden test file outputs

This certainly should be revisited frequently in the `hygiea` development process, particularly with regard to how and what information on project-specific types should flow to subject-matter experts using the text-base UI.

One possibility for a slightly more advanced application would be for `hygiea` to allow developers to use Dhall to specify the fields that *can* be tested, and to provide that to subject matter experts. That would become the common reference structure for test output, allowing the underlying `OutputData` to be more generic.

=== `Testable` typeclass
Project programmers must define how project-specific types map to the relevant testable input/output types, by implementing `Witch.From` instances.  At the moment this seems like it would offer the best compromise, despite adding some work for project developers. It
...
=== Discussion
From the stat-dev meeting on Feb. 7:

. add an `Input` type to `Hygiea.Planning`, which should mimic the `Event` type
