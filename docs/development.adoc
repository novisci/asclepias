...
* In some circumstances, a file named `Setup.hs` in the package root directory containing instructions for various setup tasks. In more detail, this file is only needed when the `build-type` field in the `.cabal` file is specified as `Custom` as described in https://cabal.readthedocs.io/en/3.6/cabal-package.html#pkg-field-build-type[the cabal documentation].
