...
This safety mechanism breaks down when you allow the compiler to choose any
name it wants by setting the name to be a type variable rather than an explicit
string.

For example, the following version compiles just fine, with `x, y` as above.
