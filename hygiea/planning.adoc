...
For example, if the `Input` type is a placeholder for `hygiea`'s `Event` implementation, the class might be defined as 

[source,haskell]
----
class (From b Input, From d Outcome) => Testable b d | b -> d where
  genOutcome :: b -> d
----
