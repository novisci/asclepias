...
include::partial$install-ghcup.adoc[leveloffset=+1]
...
So suppose that the active version of GHC was GHC {ghc}, 
then running the command `ghc` in your shell would run GHC {ghc} 
because the `+${HOME}/.ghup/bin/ghc+` file would a symbolic link pointing to GHC {ghc}.
...
----
ghcup install ghc {ghc}  # double-check to ensure this is the correct version
ghcup install cabal
----
