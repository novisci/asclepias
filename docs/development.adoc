...
GHCi provides support for fine-grained control over what top-level declarations are available in the session (i.e. what is in scope). To fully understand how the provided mechanisms work, we will first need a basic understanding of Haskell modules and how they are used to manage namespacing.
...
Since this section requires some prior knowledge of Haskell to fully understand we have included two subsections, one which can be read during a first pass, and another which can be read at a later time if desired.
