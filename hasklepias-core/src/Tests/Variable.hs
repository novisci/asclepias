...
rVarScalar = [SomeRTypeRep $ as_logical True,
...
rList = V.fromList $ zipWith (curry Just)
  ["my_logical", "my_integer", "my_numeric", "my_character"]
  rVarScalar

rVar :: [SomeRTypeRep]
rVar = [SomeRTypeRep $ as_logical [True],
...
rVarFactor :: [Variable]
rVarFactor = [rFactor "my_logical" (as_logical $ replicate 3 True) $ V.singleton (pack "True"),
             rFactor "my_integer" (as_integer [1 :: Int32, 2]) $ V.map (pack . show) $ V.fromList [1, 2],
             rFactor "my_character" (as_character $ map show [1 :: Int32, 2]) $ V.map (pack . show) $ V.fromList [1, 2],
             rFactor "my_character_2" (as_character cs) cs
             ]
               where cs = V.map (pack . show) $ V.fromList ['a'..'d']
...
rVarStype :: [Variable]
rVarStype = [stypeVector "my_binary" $ as_v_binary True dummyStypeAttrs,
            ...
            stypeVector "my_ordered" $ v_ordered (as_integer [1 :: Integer, 2]) (V.fromList $ map pack ["1", "2"]) dummyStypeAttrs]
...
rVarStypeTypesExp = map (pack . show) [LGLSXP, REALSXP, INTSXP, REALSXP, STRSXP]
...
{- Tests -}
tests :: TestTree
tests = testGroup "Variable module tests"
