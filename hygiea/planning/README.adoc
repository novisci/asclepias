...
1. Define which types exported from `asclepias` packages are allowed to be tested.
...
3. Prototype a barebones internal type system in `hygiea` that provides the 'glue' between the user interface, the test interface and the `asclepias` types 
4. Prototype a simple working example of mapping user text and project code into this type system, perhaps with text input being in dhall format for convenience

Note the goals do *not* include actually running the tests. Dhall is also probably not the desired format for user input. See the example below.
